...
export class CotizadorComponent implements OnInit {
  ...
  alElegirNombreSugerido(event: any) {
    ...
    const cliente = this.clientesBD.find(
      c => c.nombre_razon_social === nombreElegido
    );
  ...
  async buscarDocumento() {
    ...
    if (clienteExistente) {
      ...
      this.messageService.add({
        severity: 'success',
        summary: 'Local',
        detail: 'Datos obtenidos de tu Base de Datos.'
      });
      this.cdr.detectChanges();
      return;
    }
    ...
    if (doc.length !== 8 && doc.length !== 11) {
      this.messageService.add({
        severity: 'warn',
        summary: 'Error',
        detail: 'DNI (8) o RUC (11) inválido.'
      });
      return;
    }
    ...
    this.messageService.add({
      severity: 'info',
      summary: 'Consultando',
      detail: 'Buscando en SUNAT/RENIEC...'
    });
    ...
    try {
      ...
      this.clienteDireccion =
        datos.direccion_completa || datos.direccion || datos.address || '';
      ...
      this.messageService.add({
        severity: 'success',
        summary: 'API',
        detail: 'Datos obtenidos de SUNAT/RENIEC.'
      });
    } catch (e) {
      this.messageService.add({
        severity: 'error',
  ...
  async procesarClienteSilencioso() {
    if (!this.clienteNombre || !this.clienteDocumento || !this.empresaActiva?.id) {
      return;
    }

    const existe = this.clientesBD.find(
      c => c.documento_identidad === this.clienteDocumento
    );
  ...
  alElegirProductoSugerido(event: any, item: any) {
    const producto = this.productosBD.find(
      p => p.descripcion === (event.value || event)
    );
  ...
  recalcularTodo() {
    this.subtotalGeneral = this.carrito.reduce(
  ...
  async generarPDF() {
    ...
    try {
      ...
      const folioSeguro = await this.supabaseSvc.obtenerSiguienteFolio(
      ...
      this.messageService.add({
        severity: 'success',
        summary: 'Correcto',
        detail: 'Cotización generada con éxito.'
      });
    } catch (error) {
      ...
      this.messageService.add({
        severity: 'error',
        summary: 'Error',
        detail: 'No se pudo generar la cotización.'
      });
    }
  }
}