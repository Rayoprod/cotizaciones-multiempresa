...
export class PdfService {
  ...
  formatearTextoLargo(texto: string): string {
    if (!texto) return '';
    return texto.split(' ')
      .map(palabra => palabra.length > 25 ? palabra.match(/.{1,25}/g)?.join('\u200B') : palabra)
      .join(' ');
  }
  ...
  async generarYDescargarCotizacion(
    ...
    const filasTotales: any[] = [
      [{ text: 'Subtotal:', colSpan: 5, alignment: 'right', bold: true, fontSize: 10, margin: [0, 5, 0, 0] },
       '', '', '', '',
       { text: `S/ ${Number(data.subtotal).toFixed(2)}`, alignment: 'right', fontSize: 10, margin: [0, 5, 0, 0] }]
    ];

    if (tieneIgv) {
      filasTotales.push([
        { text: 'IGV (18%):', colSpan: 5, alignment: 'right', bold: true, fontSize: 10 },
        '', '', '', '',
        { text: `S/ ${Number(data.igv).toFixed(2)}`, alignment: 'right', fontSize: 10 }
      ]);
    }
    ...
    filasTotales.push([
      ...
      '', '', '', '',
      { text: `S/ ${Number(data.total).toFixed(2)}`, alignment: 'right', bold: true, fontSize: 12, color: datosEmpresa.color }
    ]);
    ...
    const docDefinition: any = {
              ...
              logoIzquierda,
              {
                ...
                text: [
                  ...
                  ...(datosEmpresa.razon_social
                    ? [{ text: `${datosEmpresa.razon_social}\n`, fontSize: 9, alignment: 'right' }]
                    : []),
      ...
      footer: (currentPage: number, pageCount: number) => ({
        ...
        alignment: 'center', fontSize: 8, color: '#9ca3af'
      }),
      content: [
        {
          style: 'clienteBox',
              ...
              columns: [
                {
                  ...
                  text: [
                    ...
                    { text: 'RUC/DNI: ', bold: true, fontSize: 9 },
                    { text: `${data.cliente_documento || '-'}`, fontSize: 9 }
                  ]
                },
                {
                  width: '40%',
                  text: [
                    ...
                    { text: '-\n', fontSize: 9 },
                    { text: 'Correo: ', bold: true, fontSize: 9 },
                    { text: '-', fontSize: 9 }
                  ],
                  alignment: 'right'
                }
          ...
          stack: [
            {
              ...
              layout: {
                ...
                hLineColor: (i: number, node: any) =>
                  (i === 0 || i === 1 || i === node.table.body.length) ? datosEmpresa.color : '#d1d5db',
                paddingTop: () => 5,
                paddingBottom: () => 5
              }
            },
          ...
          stack: [
            {
              table: {
                ...
                body: [
                  [
                    { text: 'DATOS DE ENTREGA Y OBSERVACIONES', style: 'boxHeader' },
                  ...
                  [
                    {
                      stack: [
                        ...
                        { text: [{ text: '• Notas: ', bold: true }, { text: obsFinal || 'Ninguna especificación adicional.' }] }
                      ],
                      style: 'boxContent'
                    },
                    {
    ...
    // Nombre del archivo usa empresa_id en lugar de data.empresa (que ya no existe)
    const nombreEmpresa  = (datosEmpresa.nombre_comercial || datosEmpresa.id || 'empresa').replace(/\s+/g, '_');
