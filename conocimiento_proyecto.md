...
- **Auditoría Exhaustiva de Integración Supabase-PostgreSQL & E2E**:
  - **Reparación Crítica de RPC `getnextfolioempresa`**: Corregido fallo silencioso RLS (error 42501) al agregar `SECURITY DEFINER` y `SET search_path = public` a las funciones `getnextfolioempresa` y `get_next_folio_empresa`. Esto restituyó la generación atómica y secuencial de folios (`OT-00000001`, `OT-00000002`, etc.) evitando el fallback a folios aleatorios con timestamp.
  - **Limpieza de Registros Huérfanos y Claves Foráneas**: Identificados y eliminados 3 registros huérfanos en `usuario_empresa` pertenecientes a usuarios eliminados de `profiles`. Creadas e impuestas las claves foráneas `usuario_empresa_usuario_id_fkey` (`usuario_id -> profiles(id) ON DELETE CASCADE ON UPDATE CASCADE`) y `fk_cotizaciones_cliente` (`cliente_id -> clientes(id) ON DELETE SET NULL ON UPDATE CASCADE`), además de estandarizar `ON UPDATE CASCADE` en todas las relaciones dependientes de `empresas(id)`.
  - **Imposición de Restricciones UNIQUE a Nivel de Base de Datos**: Creadas las restricciones únicas compuestas `uq_cotizaciones_empresa_folio` (`empresa_id, folio`), `uq_productos_empresa_sku` (`empresa_id, codigo_sku`) y `uq_clientes_empresa_documento` (`empresa_id, documento_identidad`), garantizando la integridad de datos multiempresa ante condiciones de carrera o peticiones concurrentes.
  - **Disparador Unificado Automático de Horómetro & Mantenimiento**: Implementada la función disparadora `fn_sync_horometro_maquinaria()` en PostgreSQL ejecutada `AFTER INSERT OR UPDATE OR DELETE` sobre `lecturas_horometro`, recalculando automáticamente `horometro_actual` y `ultimo_mantenimiento` en `maquinaria` incluso ante eliminaciones o ediciones de lecturas históricas.
  - **RPC Atómica de Eliminación de Usuarios**: Creada la función RPC `admin_eliminar_usuario(p_user_id uuid)` con `SECURITY DEFINER` para eliminar de forma limpia y transaccional los registros en `usuario_empresa`, `public.profiles` y `auth.users`, integrada en `SupabaseService.eliminarUsuario()`.
  - **Batería de Pruebas E2E CRUD (11/11 PASSED)**: Ejecutada la suite de pruebas E2E automatizada verificando la generación atómica de folios, prevención de duplicados por UNIQUE, reactividad de triggers ante eliminaciones de horómetro y borrado en cascada sin huérfanos.
