...
- **Solución Definitiva al Bloqueo del Menú Sanguchito a <= 640px**:
   - **Diagnóstico del Bug**: Al reducir la pantalla a `<= 640px`, la regla `@media screen and (max-width: 640px)` en `styles.scss` reposicionaba el contenedor `<p-toast>` a `top: 0.75rem` con `width: calc(100vw - 1.25rem)` y `z-index: 1100`. Debido a que PrimeNG renderiza el wrapper `<p-toast>` en el DOM (incluso estando vacío), este contenedor invisible con z-index 1100 se superponía exactamente sobre la barra superior (`z-index: 100`) interceptando todos los clics y toques táctiles dirigidos al botón del menú sanguchito y pill de empresa.
   - **Solución Aplicada**: Se reforzaron las reglas CSS en `styles.scss` asignando `pointer-events: none !important` al contenedor global `.p-toast` y a **todos** sus descendientes directos/indirectos vacíos (`.p-toast *`), restringiendo `pointer-events: auto !important` únicamente al mensaje visible (`.p-toast .p-toast-message`). Adicionalmente, se aisló la regla de top para que no afecte a `.p-toast-bottom-center` (el toast de actualización PWA), manteniéndolo anclado al borde inferior (`bottom: calc(1rem + env(safe-area-inset-bottom))`).
   - **Layout Robusto**: Se agregó `flex-shrink-0` al botón del sanguchito en `layout.component.html` y `admin-layout.html`, así como contención con `text-overflow-ellipsis` y `max-width` responsivo en la pill de empresa y títulos, garantizando que el menú responda al 100% en cualquier resolución de pantalla (desde 320px hasta pantallas ultra-wide).
