...
- **Corrección de Bug en Desestructuración de Registro de Usuarios en UsuariosComponent**: Corregido error en `UsuariosComponent.crearUsuario()` donde se desestructuraba erróneamente `{ data, error }` sobre el retorno plano de `SupabaseService.crearUsuario()`. Se ajustó la captura de `authData.user?.id` y se incorporó `cdr.markForCheck()` en el bloque `finally` para garantizar la inserción del perfil y la liberación de la UI en la estrategia OnPush.
