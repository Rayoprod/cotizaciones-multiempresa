...
- **Unificación de Barra de Estado del Dispositivo (Safe Area / Top Notch) y Ampliación de Pill Selector de Empresa en PWA**:
    - **Unificación de Barra Superior**: Configurado `background-color: #0f172a !important;` en los selectores globales `html` y `body` en `styles.scss`, alineando el fondo raíz de la app con el degradado ejecutivo del `.pwa-top-header` y con la etiqueta `<meta name="theme-color" content="#0f172a">`. Esto erradicó cualquier difuminación o franja blanca/azul no deseada en el área del notch / status bar en iOS Safari y Android PWA.
    - **Ampliación del Selector de Empresa en Móviles**: Al haber removido previamente insignias redundantes del encabezado, se aprovechó el ancho horizontal en teléfonos móviles asignando `max-width: clamp(210px, 68vw, 420px)` y `flex-1` a `.pwa-company-pill`. Se incrementó el contenedor circular del logo a `2.25rem` (36px) con tipografía `text-sm sm:text-base` y badge RUC claro en `text-sky-200`, permitiendo la lectura completa de los nombres comerciales de empresa sin truncamientos incómodos.
